...
import { DEFAULT_BUILDER_TEXT } from './builder-text';
 ...
 */
describe('the published builder text keys and the templates agree', () => {
  const LIB = __dirname;
  const CALL = 'ui.text(';
   ...
   * call — `ui.text(isDrifted(f) ? 'driftDetected' : 'referencedFieldTooltip')` — so the
   * scan tracks parentheses and stops at the comma that ends the argument, not the first one
   * it meets.
   */
  ...
  for (const key of Object.keys(DEFAULT_BUILDER_TEXT)) {
  ...
  it('renders every key it publishes', () => {
    expect(Object.keys(DEFAULT_BUILDER_TEXT).filter(key => !named.has(key))).toEqual([]);
  });
  ...
  it('publishes every key it renders', () => {
    ...
    const published = new Set(Object.keys(DEFAULT_BUILDER_TEXT));
